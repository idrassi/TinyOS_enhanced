...
#include "ssh.h" /* SSH Server (v1.18) */
#include "env.h"
...
void kernel_main(uint32_t magic, uint32_t info_ptr) {
     ...
     *
     * Uses Intel RDRAND if available, falls back to entropy pool
     *
    ...
    kprintf("\n");
    kprintf("*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n");
    kprintf("  TinyOS v1.0 (Build 20251110)\n");
    kprintf("*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*\n");
    kprintf("\n");
    ...
    /*=========================================================================
     * PHASE 7: NETWORK INITIALIZATION (AFTER INTERRUPTS READY!)
     *=========================================================================*/
    ...
    /*=========================================================================
     * PHASE 8: DHCP CLIENT INITIALIZATION
     *=========================================================================*/
    /*
    ...
    uint32_t dhcp_max_ticks = 1000;  // 10 seconds at 100 Hz
    ...
    if (dhcp_is_configured()) {
        ...
        kprintf("[NET] Assigned IP: 0.0.0.0 (no DHCP)\n");
    }
     ...
     * 3. Allow SSH (TCP 22) - for remote access
     * 4. Allow established connections - for response packets
     * 5. Allow ICMP - for ping/diagnostics
     *=======================================================================*/
    ...
    firewall_allow_port(22, IPPROTO_TCP, "SSH Server");
    kprintf("[FIREWALL]   - SSH (TCP port 22): ALLOW\n");
    ...
    /* SSH server initialization moved to post-login for security */
    /* Use 'sshd start' command after login to enable SSH access */

    /* DISABLED: secure_delete causing intermittent crashes */
    /* kprintf("[SECURE_DELETE] Initializing............ [OK]\n"); */
    /* secure_delete_init(); */

    /* Initialize secure boot (disabled by default for development) */
    secure_boot_init(NULL, 0, SECURE_BOOT_FLAG_MEASURED | SECURE_BOOT_FLAG_AUDIT_LOG);

    /*=========================================================================
     * PHASE 10: USER SYSTEM, VFS AND RAM FILESYSTEM INITIALIZATION
     *=========================================================================*/
    user_init();  /* Initialize user/group database (v1.10) */
    vfs_init();
    ramfs_init();
    ramfs_vfs_init();  /* Register RAMFS as VFS driver */
    user_create_etc_structure();  /* Create /etc directory structure (v1.10) */

    /* Load hello.elf into the filesystem (silent) */
    ramfs_mkdir("/hello");
    int fd = ramfs_open("/hello/hello.elf", RAMFS_FLAG_WRITE);
    if (fd >= 0) {
        ramfs_write(fd, hello_elf_data, hello_elf_data_len);
        ramfs_close(fd);
    }

    /*=========================================================================
     * DEMO: Create test files to show permissions
     *=========================================================================*/
    kprintf("\n[DEMO] Creating test files with permissions...\n");

    /* Create test files */
    fd = ramfs_open("/readme.txt", RAMFS_FLAG_WRITE);
    if (fd >= 0) {
        const char* msg = "Welcome to TinyOS!\n";
        ramfs_write(fd, msg, strlen(msg));
        ramfs_close(fd);
    }

    fd = ramfs_open("/config.conf", RAMFS_FLAG_WRITE);
    if (fd >= 0) {
        const char* msg = "system=TinyOS\nversion=1.0\n";
        ramfs_write(fd, msg, strlen(msg));
        ramfs_close(fd);
    }

    ramfs_mkdir("/documents");

    /* Change some permissions for demo */
    ramfs_chmod("/config.conf", 0600);  /* rw------- */
    ramfs_chmod("/documents", 0755);    /* rwxr-xr-x */

    /* Display the files with permissions */
    kprintf("\n");
    ramfs_list("/");
    kprintf("\n");

    /* DISABLED: Test secure deletion - causing intermittent crashes */
    ramfs_mkdir("/tmp");  /* Create /tmp for other uses */
    /* secure_delete_test(); */
    ...
    /*=========================================================================
     * PHASE 11: MULTITASKING INITIALIZATION (Silent)
     *=========================================================================*/
    ...
    int pid_ssh = task_create_kernel(task_ssh_server, "SSHServer");
    if (pid_ssh < 0) {
        kprintf("ERROR: Failed to create SSH server task\n");
        for(;;) __asm__ volatile("hlt");
    }
...
#endif
    task_set_priority(task_get((uint32_t)pid_ssh), PRIORITY_NORMAL);            // SSH server
...
#endif
    scheduler_add_task(task_get((uint32_t)pid_ssh));
    if (pid_user >= 0) {
